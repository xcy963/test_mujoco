...
u16 Get_CRC16_Check_Sum(u8* pchMessage, u32 dwLength, u16 wCRC) {
    ...
    while (dwLength--) {
        chData = *pchMessage++;
        (wCRC) = ((u16)(wCRC) >> 8) ^
                 CRC16_Table[((u16)(wCRC) ^ (u16)(chData)) & 0x00ff];
    }
    return wCRC;
}
...
u32 Verify_CRC16_Check_Sum(u8* pchMessage, u32 dwLength) {
    ...
    但是还有一个问题就是我们的数据是从低地址向着高地址放的，所以实际上我们所谓的数据地址上第一个字节记录的应该是数据的低位，也就是你写出来靠近右手边的那位
    所以他不是反的*/
    /*所以我们的>> 反映的是你写出来左手边的那部分
              &  是右手边的那部分
    但是字节数组里面 len-2对应的是内存中的低部分是你写出来右手边那部分
                  len-1对应的是内存中的高地址部分是你写出来左手边那部分
    */
}
